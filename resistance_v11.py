...
def identify_color_from_hsv(h: float, s: float, v: float) -> Tuple[str, float]:
    ...
    if 10 <= h <= 40 and s > 50 and 70 <= v <= 230:
        candidates.append(('or', 80 - abs(h - 22) * 1.5))
    ...
    if s > 40:
        ...
        # MARRON
        if 0 <= h <= 25 and v < 130:
            candidates.append(('marron', 75 - abs(h - 10) * 2))
        ...
        # JAUNE
        if 20 <= h <= 50 and s > 60:
            candidates.append(('jaune', 85 - abs(h - 30) * 1.5))
...
class ImageEnhancer:
    ...
    def run(self) -> np.ndarray:
        """Affiche la fenêtre et retourne l'image améliorée."""
        ...
        print()
        print("  Appuyez sur ENTRÉE quand l'image vous convient")
        print("  Appuyez sur R pour réinitialiser les paramètres")
        print("=" * 65)
...
def main():
    ...
    selector = BandSelector(enhanced)
    bands = selector.run()
